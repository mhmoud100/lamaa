...
export class RequestViewInfoComponent implements OnInit {
  ...
  getMapAddress(points: {lat: number, lng: number }[]): string {
    ...
    return `https://maps.googleapis.com/maps/api/staticmap?size=800x300${points.map(x=>{ return `&markers=color:blue%7Clabel:${points.indexOf(x) + 1}%7C${x.lat},${x.lng}` }).join('')}&key=${this.currentConfigService.currentConfig?.currentConfiguration.adminPanelAPIKey}`;
  }
}
