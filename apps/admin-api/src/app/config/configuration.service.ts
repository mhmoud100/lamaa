...
export class ConfigurationService {
    ...
    async updatePurchaseCode(code: string): Promise<UpdatePurchaseCodeResult> {
        const result = await firstValueFrom(this.httpService.get<{ status: 'OK' | 'USED' | string, clients?: string[] }>(`http://31.220.15.49:9000/verify?purchaseCode=${code}&port=${process.env.MAIN_PORT}`));
